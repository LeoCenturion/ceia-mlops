from src.pipeline import HierarchicalImputer, CoordinateTransformer, WindDirectionTransformer, DropColumnsTransformer, RainTodayTransformer, ExpandDateTransformer
from src.persistence.pickle_repository import PickleRepository
from src.persistence import ModelRepository
import pandas as pd
import numpy as np
import pandas as pd
...
class Model:
    ...
    def city_coords(self) -> Dict[str,tuple[float,float]]:
        return {
    ...
    def make_model(self, coordinates:pd.DataFrame = None, city_coordinates:Dict[str,tuple[float,float]] = None) -> Pipeline:
        ...
        if city_coordinates is None:
            city_coordinates = self.city_coords()
    ...
    def train(self, X: pd.DataFrame, Y: pd.Series, model: Pipeline = None, name: str = None) -> Pipeline:
        if model is None:
    ...
    def load(self, model: Pipeline = None, name: str = None) -> Pipeline:
        if model is None:
            model = self.make_model()
        if name is None:
            name = self.name
...
#     persistence_local = PickleRepository()
